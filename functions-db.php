...
/**
 * Get the number of fails by ip in last 5 minutes
 */
function ls_db_get_num_fail_by_ip_in_5_mins( $ip = '' ) {
    ...
    // set the query
    $sql    = 'SELECT count(*) as numero FROM wp_login_access WHERE wla_ip = "%s" AND wla_is_failed = 1 AND wla_date > "%s"';

    $sql_prepare = $wpdb->prepare($sql,$ip,date("Y-m-d H:i:s",strtotime("-5 minutes")));

    // execute the query
    $num    = $wpdb->get_results($sql_prepare);
